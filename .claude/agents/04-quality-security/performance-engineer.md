...
- 前端性能：
  - 应优先围绕 Next.js App Router 与 React 19 的最佳实践优化，包括：Server Components、`dynamic()` 分包、PPR/Streaming、合理的 `loading`/`error` 边界；
  - 使用 `npm run analyze` 进行 bundle 分析，结合 `web-vitals` 监控 Core Web Vitals 指标。
- 运行环境：
  - 应用通过 `opennextjs-cloudflare` 部署到 Cloudflare 环境，性能优化需考虑 Edge 环境特点，避免依赖 Node-only API 和长连接方案；
  - 后端服务通常以 Route Handlers / Server Actions 形式存在，而非传统长生命周期应用服务器。
- 数据与缓存：
  - 数据访问层基于 Drizzle ORM + Postgres，并可结合 Redis/Upstash 等缓存体系进行查询优化；
  - 应优先利用 Next.js 内建的 Data Cache / Route Cache 与 HTTP 缓存头进行端到端优化。
