...
- 前端：Next.js 15 App Router（`src/app`），React 19，Tailwind CSS + Shadcn UI；多语言通过 `next-intl` 和 `src/app/[locale]/**` 路由实现。
- 后端：基于 Next.js Route Handlers / Server Actions，不单独起 Express/Koa 服务；
  - 数据访问使用 Drizzle ORM + Postgres（`drizzle-orm`, `postgres`），必要时结合 Redis/Upstash 做缓存；
  - 认证基于 `better-auth` 与 Next.js 中间件/Session 实现。
- 部署：使用 `opennextjs-cloudflare` 目标到 Cloudflare 环境，需避免依赖 Node-only API（如 `fs` 等）出现在运行时路径上。
- 推荐的开发/检查命令：
  - 开发：`npm run dev` 或 `npm run dev:fast`；
  - 类型检查：`npm run type-check`；
  - Lint & 格式：`npm run lint` / `npm run format`；
  - 数据库变更：`npm run db:generate` + `npm run db:migrate`；
  - 生产构建与预览/部署：`npm run build`、`npm run preview`、`npm run deploy`。
- 设计全栈特性时，应优先：
  - 在 App Router 下定义 API（`app/api/**/route.ts`）或 Server Actions；
  - 通过共享 TypeScript 类型在前后端之间传递结构化数据，而不是重复定义接口模型。
