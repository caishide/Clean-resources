...
- 单元与集成测试：
  - 默认使用 **Vitest**（含 `@testing-library/react` 等）进行前后端及组件测试；
  - 常用脚本：`npm run test`、`npm run test:unit`、`npm run test:modules`、`npm run test:coverage`。
- 端到端测试：
  - 使用 **Playwright**（`@playwright/test`）对 Next.js App Router 应用进行浏览器级 E2E 测试；
  - 常用脚本：`npm run test:e2e`、`npm run test:e2e:ui`、`npm run test:e2e:headed`。
- 其他专项：
  - 安全/接口/多语言等专项测试已有对应脚本（如 `npm run test:security`、`npm run test:api`、`npm run test:e2e:i18n`），应在设计自动化策略时一并考虑。
- 浏览器自动化优先选择 Playwright，而非在本项目中引入 Selenium/Cypress，除非有明确兼容性需求。
