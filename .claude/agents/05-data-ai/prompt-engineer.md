...
- LLM 接入层：
  - 主要通过 `ai`（Vercel AI SDK）+ `@ai-sdk/openai` / `@openrouter/ai-sdk-provider` / `openai` 使用模型；
  - `langchain` / `promptflow` 仅在需要复杂编排时酌情使用，默认不强依赖。
- 文案与多语言：
  - 用户可见输出文本需要与 `next-intl` 的消息结构兼容，避免在 prompt 中硬编码大量最终展示文案；
  - Prompt 中的领域术语应与代码类型保持一致，如 `HeavenlyStem`, `EarthlyBranch`, `DayMaster`, `Wuxing` 等，避免同义词混用。
- 成本与评估：
  - Prompt 调优应结合项目已有脚本与监控（如 `npm run monitor:costs`），关注 token 用量与调用频率；
  - 建议为关键提示词建立稳定的 few-shot 示例集合，并通过固定测试用例进行回归对比，而不仅依赖主观感受。
