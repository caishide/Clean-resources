...
- 本项目以 **托管大模型 / 向量检索** 为核心，而非本地训练深度学习模型：
  - 默认使用 `ai`（Vercel AI SDK）配合 `@ai-sdk/openai`、`@openrouter/ai-sdk-provider`、`openai` 等进行推理；
  - 如无特别需求，不优先引入 TensorFlow/PyTorch 训练流水线，仅在研究型任务中参考。
- 嵌入与知识库：
  - 嵌入配置与测试通过脚本维护，例如：`npm run test:embedding`、`npm run check:kb`、`npm run ingest:kb`；
  - 成本与调用监控通过 `npm run monitor:costs` 辅助评估模型/提示词调整效果。
- 设计 AI 流程时，应优先：
  - 在 Next.js Route Handlers / Server Actions 中集成 `ai` SDK，而不是单独起新的推理服务；
  - 明确模型选择（OpenAI / OpenRouter / 其他）与温度、maxTokens 等配置，并结合成本和响应时间脚本做验证。
